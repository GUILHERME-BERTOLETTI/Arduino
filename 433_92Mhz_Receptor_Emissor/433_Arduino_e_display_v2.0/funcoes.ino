...
void MENU_1(){
 ...
 if (digitalRead(botao1) == LOW ) {
    while (digitalRead(botao1) == LOW){} // Esse while garante que se o botao ficar pressionado nada acontece, somente quando for solto pra nao gerar loop
    if (cont == 5) cont = -1;
  cont++;
  delay(200);
  lcd.clear();
...
// Se botao2 for pressionado vai para nivel baixo e entra neste if
  if (digitalRead(botao2) == LOW) {
    while (digitalRead(botao2) == LOW){} // Esse while garante que se o botao ficar pressionado nada acontece, somente quando for solto pra nao gerar loop
    if (cont == 0) cont = 6;
  cont--;
  delay(200);
  lcd.clear();
